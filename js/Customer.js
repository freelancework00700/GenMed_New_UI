app.controller('CustomerController', function ($scope, $compile, $rootScope, $http, $localstorage, $ionicPopup, $state, $ionicLoading, $ionicScrollDelegate) {
    ...
    $scope.FilterData = function () {
        $('#CustTable').dataTable().api().ajax.reload();
    ...
    function InitDataTable() {
        ...
        $('#CustTable').DataTable({
            ...
            "aaSorting": [1, 'ASC'],
            "ajax": {
                url: $rootScope.RoutePath + 'customer/GetAllDynamicCustomer',
                data: function (d) {
    ...
    $scope.SaveCustomer = function (form) {
        if (form.$invalid) {
            ...
            // if ($scope.IsAdmin) {
            //     $scope.model.CreatedBy = ''
            // }
            console.log($scope.model);
            $rootScope.ShowLoader();
            $http.post($rootScope.RoutePath + 'customer/SaveCustomer', $scope.model)
                .then(function (res) {
                    console.log(res);
    ...
    $scope.CopyToModel = function (idCust) {
        ...
        $scope.model.CreatedDate = moment($scope.model.CreatedDate).format('YYYY-MM-DD')
    ...
    $scope.Export = function () {
        var params = {
            id: $scope.IsAdmin ? '' : parseInt($localstorage.get('UserId')),
            StartDate: $scope.modelAdvanceSearch.StartDate != '' && $scope.modelAdvanceSearch.StartDate != undefined ? moment($scope.modelAdvanceSearch.StartDate).set({ hour: 0, minute: 0, second: 0 }).format('YYYY-MM-DD') : '',
            EndDate: $scope.modelAdvanceSearch.EndDate != '' && $scope.modelAdvanceSearch.EndDate != undefined ? moment($scope.modelAdvanceSearch.EndDate).set({ hour: 0, minute: 0, second: 0 }).format('YYYY-MM-DD') : ''
        }
        window.location = $rootScope.RoutePath + "customer/ExportCustomerData", { params: params };
    }
    ...
    $scope.Cancel = function () {
        ...
        $scope.IsEdit = false;
        $scope.formsubmit = false;
        $('#CustTable').dataTable().fnClearTable();
        $('#CustTable').dataTable().fnDestroy();
        setTimeout(function () {
            InitDataTable();
        })
    }
