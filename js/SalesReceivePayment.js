app.controller('SalesReceivePayment', function ($scope, $http, $rootScope, $ionicLoading, $ionicPopup, $localstorage, $compile, $ionicScrollDelegate) {
    ...
    $scope.Export = function () {
        ...
        var Param = "?CustomerCode=" + $scope.model.CustomerCode;
        window.location = $rootScope.RoutePath + "salesreceivepayment/ExportSalesReceivePayment" + Param;
    }

    $scope.GetAllUnPayARInvoiceByCustomerCode = function () {
        $scope.lstKnockOff = [];
        $http.get($rootScope.RoutePath + 'salesreceivepayment/GetAllUnPaySalesReceiveByCustomerCode?CustomerCode=' + $scope.model.CustomerCode).then(function (res) {
            $scope.lstInvoice = res.data.data;
            console.log($scope.lstInvoice)
            if ($scope.lstInvoice.length > 0) {
                for (var i = 0; i < $scope.lstInvoice.length; i++) {
                    var obj = new Object();
                    obj.id = 0;
                    obj.IdARPayment = null;
                    obj.KnockOffDocType = 'RI';
                    obj.KnockOffDate = null;
                    obj.Amount = 0.00;
                    obj.DiscountAmount = 0.00;
                    obj.IsDiscount = false;
                    obj.idInvoice = $scope.lstInvoice[i].id;
                    obj.Total = $scope.lstInvoice[i].NetTotal;
                    obj.Outstanding = $scope.lstInvoice[i].Outstanding;
                    obj.DocNo = $scope.lstInvoice[i].DocNo;
                    obj.InvoiceDate = moment($scope.lstInvoice[i].InvoiceDate).format('DD-MM-YYYY');
                    obj.IsSelected = false;
                    obj.KnockOffDetail = [];

                    for (var j = 0; j < $scope.lstInvoice[i].invoicedetails.length; j++) {
                        var obj1 = {
                            id: 0,
                            IdARPaymentKnockOff: null,
                            Amount: $scope.lstInvoice[i].invoicedetails[j].NetTotal,
                            LocalPaymentAmount: $scope.lstInvoice[i].invoicedetails[j].NetTotal,
                            LocalInvoiceAmount: $scope.lstInvoice[i].invoicedetails[j].NetTotal
                        };
                        obj.KnockOffDetail.push(obj1);
                    }
                    $scope.lstKnockOff.push(obj);
                }
                console.log($scope.lstKnockOff)
            }
        })
    }
    ...
    $scope.SelectCustomer = function () {
        ...
        $scope.GetAllUnPayARInvoiceByCustomerCode();
    }
    ...
    function AdjustAmount() {
        ...
        console.log($scope.lstSelectedPaymentMain, $scope.lstKnockOff)
        ...
        if (obj) {
            ...
            $scope.model.LocalUnappliedAmount = $scope.model.PaymentAmount - SUM;
            _.filter($scope.lstKnockOff, function (m) {
                if (m.IsSelected == true) {
                    m.Outstanding = 0;
                }
    ...
    $scope.FilterData = function () {
        $('#ARPaymentTable').dataTable().api().ajax.reload();
    }
    ...
    $scope.CopyToModel = function (id) {
        ...
        $scope.lstSelectedPaymentMain = _.filter(selectedItem.ar_paymentdetails, function (q) {
            if (q.IsRCHQ) {
                q.IsRCHQ = true;
        ...
        var List = selectedItem.ar_payment_knockoffs;

        $http.get($rootScope.RoutePath + 'salesreceivepayment/GetAllInvoiceByPatientId?id=' + $scope.model.id).then(function (res) {
            _.filter(List, function (n) {
                var obj = new Object();
                obj.id = n.id;
                obj.IdARPayment = n.IdARPayment;
                ...
                obj.idInvoice = n.idInvoice;
                obj.IsSelected = true;
                obj.KnockOffDetail = n.ar_payment_knockoffdetails;
                console.log(res.data)
                var obj1 = _.findWhere(res.data, { id: parseInt(n.idInvoice) });
                obj.Total = obj1.NetTotal;
                obj.Outstanding = obj1.Outstanding;
                obj.DocNo = obj1.DocNo;
                obj.InvoiceDate = moment(obj1.InvoiceDate).format('DD-MM-YYYY');
    ...
    function InitDataTable() {
        if ($.fn.DataTable.isDataTable('#ARPaymentTable')) {
            $('#ARPaymentTable').DataTable().destroy();
        }
        $('#ARPaymentTable').DataTable({
            ...
            "aaSorting": [2, 'DESC'],
            "ajax": {
                url: $rootScope.RoutePath + 'invoicepayment/GetAllARPaymentDynamic',
                data: function (d) {
    ...
    $scope.formsubmit = false;
    $scope.SaveARPayment = function (form) {
        ...
        if (form.$invalid) {
            ...
            function SavePayment() {
                ...
                console.log(obj)
                $http.post($rootScope.RoutePath + 'salesreceivepayment/SaveSalesReceivePayment', obj).then(function (res) {
                    if (res.data.success) {
    ...
    $scope.DeleteItem = function (id) {
        ...
        confirmPopup.then(function (res) {
            if (res) {
                ...
                $rootScope.ShowLoader();
                $http.get($rootScope.RoutePath + 'invoicepayment/DeleteARPayment', {
                    params: params
