app.controller('DynamicPageController', function ($scope, $rootScope, $http, $ionicPopup, $ionicLoading, $localstorage, $state) {
    ...
    $scope.init = function () {
        ...
        $scope.ResetModel();
        $scope.GetAllDynamicPage();
        $rootScope.BackButton = $scope.IsList = true;
    };

    $scope.GetAllDynamicPage = function () {
        $http.get($rootScope.RoutePath + 'dynamicpage/GetAllDynamicPage').then(function (res) {
            console.log(res)
            $scope.lstDynamicPage = res.data.data;
            console.log($scope.lstDynamicPage);

            $scope.TotalRecord = $scope.lstDynamicPage.length;
            $(document).ready(function () {
                $('#DynamicPageTable').dataTable().fnClearTable();
                $('#DynamicPageTable').dataTable().fnDestroy();
                setTimeout(function () {
    ...
    $scope.FilterData = function () {
        if ($scope.Searchmodel.Search != '' && $scope.Searchmodel.Search != null && $scope.Searchmodel.Search != undefined) {
            $('#DynamicPageTable').dataTable().fnFilter($scope.Searchmodel.Search);
        } else {
            $('#DynamicPageTable').dataTable().fnFilter("");
        }
    }

    $scope.formsubmit = false;
    $scope.SaveDynamicPage = function (FormUOM) {
        if (FormUOM.$valid) {
            if (!$scope.model.id) {
                $scope.model.id = 0;
            }
            $rootScope.ShowLoader();
            $http.post($rootScope.RoutePath + "dynamicpage/SaveDynamicPage", $scope.model)
                .then(function (res) {
                    ...
                    $http.post($rootScope.RoutePath + "dynamicpage/UploadImage", formData, {
    ...
    $scope.DeleteDynamicPage = function (id) {
        ...
        confirmPopup.then(function (res) {
            if (res) {
                ...
                $rootScope.ShowLoader();
                $http.get($rootScope.RoutePath + 'dynamicpage/DeleteDynamicPage', {
                    params: params
