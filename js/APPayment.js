app.controller('APPaymentCtrl', function ($scope, $http, $rootScope, $ionicLoading, $ionicPopup, $localstorage, $compile, $ionicScrollDelegate) {
    ...
    $scope.GetAllUnPayAPInvoiceByCustomerCode = function () {
        $scope.lstKnockOff = [];
        $http.get($rootScope.RoutePath + 'purchaseinvoicepayment/GetAllUnPayAPInvoiceByCustomerCode?CustomerCode=' + $scope.model.CustomerCode).then(function (res) {
            $scope.lstAPInvoice = res.data.data;
            if ($scope.lstAPInvoice.length > 0) {
                for (var i = 0; i < $scope.lstAPInvoice.length; i++) {
                    var obj = new Object();
                    obj.id = 0;
                    ...
                    obj.IdAPInvoice = $scope.lstAPInvoice[i].id;
                    obj.Total = $scope.lstAPInvoice[i].NetTotal;
                    obj.Outstanding = $scope.lstAPInvoice[i].Outstanding;
                    obj.DocNo = $scope.lstAPInvoice[i].DocNo;
                    obj.APInvoiceDate = moment($scope.lstAPInvoice[i].APInvoiceDate).format('DD-MM-YYYY');
                    obj.IsSelected = false;
                    obj.KnockOffDetail = [];

                    for (var j = 0; j < $scope.lstAPInvoice[i].ap_invoicedetails.length; j++) {
                        var obj1 = {
                            id: 0,
                            IdAPPaymentKnockOff: null,
                            Amount: $scope.lstAPInvoice[i].ap_invoicedetails[j].NetTotal,
                            LocalPaymentAmount: $scope.lstAPInvoice[i].ap_invoicedetails[j].NetTotal,
                            LocalInvoiceAmount: $scope.lstAPInvoice[i].ap_invoicedetails[j].NetTotal
                        };
    ...
    $scope.CopyToModel = function (id) {
        ...
        $http.get($rootScope.RoutePath + 'purchaseinvoicepayment/GetAllAPInvoiceByPatientId?id=' + $scope.model.id).then(function (res) {
            _.filter(List, function (n) {
                ...
                obj.IdAPInvoice = n.IdAPInvoice;
                obj.IsSelected = true;
                obj.KnockOffDetail = n.ap_payment_knockoffdetails;

                var obj1 = _.findWhere(res.data, { id: parseInt(n.IdAPInvoice) });
    ...
    $scope.SaveAPPayment = function (form) {
        ...
        if (form.$invalid) {
            ...
            function SavePayment() {
                ...
                $http.post($rootScope.RoutePath + 'purchaseinvoicepayment/SavePurchaseInvoiceReceivePayment', obj).then(function (res) {
                    if (res.data.success) {
