app.controller('PurchaseInvoiceController', function ($scope, $rootScope, $http, $ionicPopup, $ionicLoading, $localstorage, $state, $compile, $ionicScrollDelegate) {
    ...
    $scope.SelectCustomer = function (o) {
        ...
        if (obj) {
            ...
            $scope.GetAllPurchaseOrderByCustomerCode();
        } else {
    ...
    $scope.GetAllPurchaseOrderByCustomerCode = function () {
        $http.get($rootScope.RoutePath + 'purchaseorder/GetAllPurchaseOrderByCustomerCode?CustomerCode=' + $scope.model.CustomerCode).then(function (res) {
            $scope.lstPurchaseOrder = res.data.data;
        });
    }

    $scope.ChangePurchaseModel = function () {
        var selectedItem = _.findWhere($scope.lstPurchaseOrder, { DocNo: $scope.model.PurchaseOrderDocNo });
        ...
        if (selectedItem) {
            for (var prop in $scope.model) {
                ...
                if (prop == 'PurchaseDate') {
                    $scope.model['PurchaseDate'] = moment().format('YYYY-MM-DD');
                }
                if (prop == 'CreatedBy') {
                    $scope.model['CreatedBy'] = parseInt($localstorage.get('UserId'));
                }
                if (prop == 'DocNo') {
                    $scope.model['DocNo'] = null;
                }
                if (prop == 'PurchaseOrderDocNo') {
                    $scope.model['PurchaseOrderDocNo'] = selectedItem.DocNo;
                }
            ...
            if ($scope.lstSelectedPurchaseMain.length > 0) {
                for (var i = 0; i < $scope.lstSelectedPurchaseMain.length; i++) {
                    $scope.lstSelectedPurchaseMain[i].id = 0;
                    $scope.lstSelectedPurchaseMain[i].idPurchase = null;
                    ...
                    $scope.lstSelectedPurchaseMain[i].LastBalQty = 0;
                    $scope.lstSelectedPurchaseMain[i].LastBatchNo = null;
                    $scope.lstSelectedPurchaseMain[i].LastLocation = null;
                    $scope.lstSelectedPurchaseMain[i].LastItemCode = null;
                    $scope.lstSelectedPurchaseMain[i].LastUOM = null;
                }
    ...
    $scope.SavePurchaseInvoice = function (form) {
                ...
                if ($scope.ListMaxPurchaseQty.length > 0 || $scope.ListMinPurchaseQty.length > 0) {
                    ...
                    confirmPopup.then(function (res) {
                        if (res) {
                            ...
                            $http.post($rootScope.RoutePath + 'purchaseinvoice/SavePurchaseInvoice', $scope.model).then(function (res) {
                                if (res.data.success) {
                                    $scope.init();
                                }
    ...
    $scope.CopyToModel = function (id) {
        ...
        if (selectedItem.PurchaseOrderDocNo) {
            $scope.GetAllPurchaseOrderByCustomerCode();
        }

        $scope.lstSelectedPurchaseMain = selectedItem.purchaseinvoicedetails;
    ...
    $scope.DeleteItem = function (id) {
        ...
        confirmPopup.then(function (res) {
            if (res) {
                ...
                $rootScope.ShowLoader();
                $http.get($rootScope.RoutePath + 'purchaseinvoice/DeletePurchaseInvoice', {
                    params: params
    ...
    $scope.FilterData = function () {
        $('#PurchaseInvoiceTable').dataTable().api().ajax.reload();
    }
    ...
    function InitDataTable() {
        ...
        $('#PurchaseInvoiceTable').DataTable({
            ...
            "aaSorting": [2, 'DESC'],
            "ajax": {
                url: $rootScope.RoutePath + 'purchaseinvoice/GetAllPurchaseInvoiceDynamic',
                data: function (d) {
    ...
    $scope.GenerateReport = function (Id) {
        window.open($rootScope.RoutePath + "purchaseinvoice/GenerateInvoice?Id=" + Id, '_blank');
    }
