app.controller('InvoiceReceivePaymentCtrl', function ($scope, $http, $rootScope, $ionicLoading, $ionicPopup, $localstorage, $compile, $ionicScrollDelegate) {
    ...
    $scope.GetAllUnPayARInvoiceByCustomerCode = function () {
        $scope.lstKnockOff = [];
        $http.get($rootScope.RoutePath + 'invoicepayment/GetAllUnPayARInvoiceByCustomerCode?CustomerCode=' + $scope.model.CustomerCode).then(function (res) {
            $scope.lstARInvoice = res.data.data;
            if ($scope.lstARInvoice.length > 0) {
                for (var i = 0; i < $scope.lstARInvoice.length; i++) {
                    var obj = new Object();
                    obj.id = 0;
                    ...
                    obj.IdARInvoice = $scope.lstARInvoice[i].id;
                    obj.Total = $scope.lstARInvoice[i].NetTotal;
                    obj.Outstanding = $scope.lstARInvoice[i].Outstanding;
                    obj.DocNo = $scope.lstARInvoice[i].DocNo;
                    obj.ARInvoiceDate = moment($scope.lstARInvoice[i].ARInvoiceDate).format('DD-MM-YYYY');
                    obj.IsSelected = false;
                    obj.KnockOffDetail = [];

                    for (var j = 0; j < $scope.lstARInvoice[i].ar_invoicedetails.length; j++) {
                        var obj1 = {
                            id: 0,
                            IdARPaymentKnockOff: null,
                            Amount: $scope.lstARInvoice[i].ar_invoicedetails[j].NetTotal,
                            LocalPaymentAmount: $scope.lstARInvoice[i].ar_invoicedetails[j].NetTotal,
                            LocalInvoiceAmount: $scope.lstARInvoice[i].ar_invoicedetails[j].NetTotal
                        };
    ...
    $scope.CopyToModel = function (id) {
        ...
        $http.get($rootScope.RoutePath + 'invoicepayment/GetAllARInvoiceByPatientId?id=' + $scope.model.id).then(function (res) {
            _.filter(List, function (n) {
                ...
                obj.IdARInvoice = n.IdARInvoice;
                obj.IsSelected = true;
                obj.KnockOffDetail = n.ar_payment_knockoffdetails;

                var obj1 = _.findWhere(res.data, { id: parseInt(n.IdARInvoice) });
                obj.Total = obj1.NetTotal;
                obj.Outstanding = obj1.Outstanding;
                obj.DocNo = obj1.DocNo;
                obj.ARInvoiceDate = moment(obj1.ARInvoiceDate).format('DD-MM-YYYY');
    ...
    $scope.SaveARPayment = function (form) {
        ...
        if (form.$invalid) {
            ...
            function SavePayment() {
                ...
                $http.post($rootScope.RoutePath + 'invoicepayment/SaveInvoiceReceivePayment', obj).then(function (res) {
                    if (res.data.success) {
