app.controller('DebtorOutstandReportController', function ($scope, $rootScope, $http, $ionicPopup, $ionicLoading, $localstorage, $state, $compile, $ionicModal) {
    ...
    $scope.FilterData = function () {
        $('#DebtorOutstandReportTable').dataTable().api().ajax.reload();
    }
    ...
    function InitDataTable() {
        if ($.fn.DataTable.isDataTable('#DebtorOutstandReportTable')) {
            $('#DebtorOutstandReportTable').DataTable().destroy();
        }
        $('#DebtorOutstandReportTable').DataTable({
            "processing": true,
            ...
            "aaSorting": [1, 'ASC'],
            "ajax": {
                url: $rootScope.RoutePath + 'report/GetDebtorOutstandReport',
                data: function (d) {
    ...
    $scope.openDetailModal = function (id) {
        ...
        $scope.GetAllARInvoiceByCustomerCode(obj.CustomerCode);
        $scope.detail.show();
    }
    ...
    $scope.GetAllARInvoiceByCustomerCode = function (o) {
        $http.get($rootScope.RoutePath + 'report/GetARInvoiceDetailByCustomerCode?CustomerCode=' + o).then(function (res) {
            res.data.data = _.sortBy(res.data.data, function (dates) {
                return dates.ARInvoiceDate;
            }).reverse();
    ...
    $scope.Export = function () {
        ...
        window.location = $rootScope.RoutePath + "report/ExportDebtorOutstandReport" + Param;
    }
    // Alias
