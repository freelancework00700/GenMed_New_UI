...
var intlpnCtrl = function($ionicModal, $scope, intlpnUtils) {
    ...
    self.boxHeaderClass = angular.isDefined(self.boxHeaderClass) ? self.boxHeaderClass : "";
    ...
    .directive('intlpn', ['$ionicModal', '$timeout', '$ionicScrollDelegate', function($ionicModal, $timeout, $ionicScrollDelegate) {
        return {
            ...
            link: function(scope, element, attrs, ngModelCtrl) {
                ...
                var modalTemplate = '<ion-modal-view class="offerBG tel-popup-main">' +
                    '<ion-header-bar class="' + scope.boxHeaderClass + '">' + //need to have the class before creation
                    '<h1 class="title text-white">{{modalScope.boxHeaderTitle}}</h1>' +
                    // '<button class="button button-clear icon ion-ios-close-empty modal-close-btn text-white" ng-click="modalScope.close()"></button>' +
                    '<i class="ion-close modal-close-btn text-white" ng-click="modalScope.close()"></i>' +
                    '</ion-header-bar>' +
                    '<div class="bar bar-subheader item-input-inset ' + scope.boxHeaderClass + ' " ng-if="modalScope.countries.length > 10">' +
                    ...
                    'class="item-icon-left" ng-class="(country.iso2 == modalScope.currentCountry)?\'item-icon-right\':\'\'" style="border-bottom: 1px solid #000; color: #fff;">' +
            ...
            template: '<div class="item item-input intlpn-container">' +
                ...
                '<input intlpn-formatter national-mode="nationalMode" iso-code="{{isocode}}" type="tel" ng-model="phone" >' +
                '</div>'
        };
    }])
